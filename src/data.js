...
export const categories=()=>{
  return  Array.from(new Set(emojis.map(obj=>obj.category))).sort()
}

import {v4} from 'uuid'

  export const selectedEmojis=(categ,nr)=>{
    console.log(categ,nr);
    let arr=emojis.filter(obj=>obj.category==categ)
    arr=arr.map(obj=>{
      const {emoji,description,category}=obj
      return {emoji,description,category,disabled:false}
    })
    arr.sort(()=>Math.random()-0.5) 
    arr=arr.slice(0,nr*nr/2)
    arr=[...arr,...arr]
    //legyen egységes azonosítonk is id:
    arr=arr.map(obj=>({...obj,id:v4()}))
